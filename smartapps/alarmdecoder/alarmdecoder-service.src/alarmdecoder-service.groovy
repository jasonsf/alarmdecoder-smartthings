...
def addZone(evt) {
    ...
    try {        
        def zone_switch = addChildDevice(APPNAMESPACE, "AlarmDecoder virtual contact sensor", "${evt.data}", state.hub, [name: "${evt.data}", label: "${currentSensorKey}: ${currentSensorValue}", completedSetup: true])
